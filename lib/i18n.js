...
class UIString {
  ...
  get label () {
    const componentLabel = this.component ? this.component.label : undefined
    const context = this.component ? `${this.component.type} ${this.path}` : this.path
    if (this.path && this.path.startsWith('values.')) {
      return `${this.value} (in "${componentLabel || 'unknown'}" ${context})`
    } else if (componentLabel) {
      return `${componentLabel} (${context})`
    }
    return this.key
  }

  get isPageComponent () {
    return this.component && this.component.type === 'panel'
  }

  get pageComponent () {
    return this.isPageComponent
      ? this.component
      : [].concat(this.parents).find(c => c && c.type === 'panel')
  }

  get page () {
    const { pageComponent } = this
    return pageComponent
      ? pluck(pageComponent, ['key', 'title'])
      : undefined
  }
}
